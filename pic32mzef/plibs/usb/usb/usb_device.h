...
#define AFTEVECTOR(x)               _TIMER_ ## x ## _VECTOR
#define XAFTEVECTOR(x)              AFTEVECTOR(x)
#define UTIMER_VECTOR_NUM           USB_DEVICE_UTIMER_NUMBER
#define _UTIMER_X_VECTOR            XAFTEVECTOR(UTIMER_VECTOR_NUM)

#define AFTEPRIOR(x)                IPL ## x ## AUTO
#define XAFTEPRIOR(x)               AFTEPRIOR(x)
#define UTIMER_PRIORITY             USB_DEVICE_UTIMER_INT_PRIORITY
#define IPLxAUTO                    XAFTEPRIOR(UTIMER_PRIORITY)

#define AFTEINTPRIOR(x)             UT_INT_PRIOR_ ## x
#define XAFTEINTPRIOR(x)            AFTEINTPRIOR(x)
#define UTIMER_INT_PRIORITY         USB_DEVICE_UTIMER_INT_PRIORITY
#define UT_INT_PRIOR_X              XAFTEINTPRIOR(UTIMER_INT_PRIORITY)

#define AFTEUTIMER(x)               UTIMER ## x
#define XAFTEUTIMER(x)              AFTEUTIMER(x)
#define UTIMER_NUM                  USB_DEVICE_UTIMER_NUMBER
#define USB_DEVICE_UTIMER           XAFTEUTIMER(UTIMER_NUM)

#define AFTEUTPS(x)                 UT_PS_ ## x
#define XAFTEUTPS(x)                AFTEUTPS(x)
#define UTIMER_TIMER_PRESCALE       USB_DEVICE_UTIMER_PRESCALE
#define UT_PS_X                     XAFTEUTPS(UTIMER_TIMER_PRESCALE)  
#endif        
...
typedef uintptr_t USB_DEVICE_TMR_HANDLE;

// *****************************************************************************
/* DRV USB Timer invalid handle macro definition
...
#define USB_DEVICE_TMR_HANDLE_INVALID           ((USB_DEVICE_TMR_HANDLE)(-1))

#define USB_DEVICE_TMR_HANDLE_VALID             ((USB_DEVICE_TMR_HANDLE)(1))
...
typedef struct 
{
     ...
     *  */
    USBHS_INT_SOURCE interruptSource;

    /* Interrupt Source for USB DMA module. The use of this parameter is
     * deprecated. */
    USBHS_INT_SOURCE interruptSourceUSBDma;
    ...
    /**/
    void (* appUsbTask)(void);
    
    /**/
    void (* appUsbExternalTask)(void);    
} USB_DEVICE_INIT;
...
/* Function:
  ...
  Summary:
    Disabled the specified instance of the USB device layer.
  ...
  Example:
    ...
    USB_DEVICE_Disable(usbDeviceobj); 

    </code>

  Remarks:
    Once the Initialize operation has been called, the disabled operation
    must be called before the Initialize operation can be called again.
*/
...
#ifndef  USE_UTIMER_USBDEVICE_SYS_TASK
...
#include "src/usb_device_mapping.h"
