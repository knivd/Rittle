...
#include "../../../peripheral/timer/utimer.h"

typedef struct{
    DRV_USB_TMR_HANDLE  ut_handle;
    unsigned int        ut_tickcount;
}utmr_t;  

static utmr_t usb_device_tasktmr = {.ut_handle = DRV_USB_TMR_HANDLE_INVALID, .ut_tickcount = 0};
#endif
...
// *****************************************************************************
/* Function:
    DRV_USB_TMR_HANDLE DRV_USBHS_StartTaskTimer ( UTIMER_MODULE id, uint32_t tickMs ) 
...
USB_DEVICE_TMR_HANDLE USB_Device_StartTaskTimer ( UTIMER_MODULE id, uint32_t tickMs ) 
{
    uint32_t processorStatus;
    USB_DEVICE_TMR_HANDLE ret = USB_DEVICE_TMR_HANDLE_INVALID;
    
    /* Save the processor status and then Disable the global interrupt */
    processorStatus = __builtin_disable_interrupts();    
    
    if( USB_DEVICE_TMR_HANDLE_INVALID == usb_device_tasktmr.ut_handle )
    {
        usb_device_tasktmr.ut_tickcount = tickMs;
        
        OpenUTimer(id, UT_ON | UT_SOURCE_INT | UT_PS_X, tickMs);

        // set up the timer interrupt priority
        ConfigIntUTimer(id, UT_INT_ON | UT_INT_PRIOR_X);
        
        usb_device_tasktmr.ut_handle = USB_DEVICE_TMR_HANDLE_VALID;
        ret = USB_DEVICE_TMR_HANDLE_VALID;
    }
...
// *****************************************************************************
/* Function:
    void USB_Device_StopTaskTimer ( UTIMER_MODULE id ) 
...
void USB_Device_StopTaskTimer ( UTIMER_MODULE id ) 
{
    ...
    /* stop the Interrupt of the UTimer device task" */
    mUTIntEnable(USB_DEVICE_UTIMER, false);
    
    usb_device_tasktmr.ut_handle = USB_DEVICE_TMR_HANDLE_INVALID;
...
)
{
    ...
    USB_DEVICE_TMR_HANDLE timerHandle;
    ...
    /**/
    usbDeviceThisInstance->appUsbTask = deviceInit->appUsbTask;
            
    /**/
    usbDeviceThisInstance->appUsbExternalTask = deviceInit->appUsbExternalTask;    
...
#ifdef  USE_UTIMER_USBDEVICE_SYS_TASK
    timerHandle = USB_Device_StartTaskTimer(USB_DEVICE_UTIMER, USB_DEVICE_UTIMER_TICK);
    if( timerHandle == USB_DEVICE_TMR_HANDLE_INVALID )
    {
        return (SYS_MODULE_OBJ_INVALID);
    }       
#endif    
...
// *****************************************************************************
/* Function:
    void USB_DEVICE_Disable ( SYS_MODULE_OBJ usbDeviceobj)

  Summary:
    Disable the specified instance of the USB device layer.

  Description:
    This function reset the specified instance of the USB device layer,
    disabling its operation (and any hardware) and invalidates all of the
    internal data.
...
#ifdef  USE_UTIMER_USBDEVICE_SYS_TASK
void __ISR(_UTIMER_X_VECTOR, IPLxAUTO) DRV_UTaskTimerHandler(void)
{
USB_DEVICE_OBJ * usbDeviceThisInstance;

    // clear the interrupt flag
    mUTClearIntFlag(USB_DEVICE_UTIMER);
    
    /* Get this instance of USB device layer.*/
    usbDeviceThisInstance = &usbDeviceInstance[0];    
    
    /* USB HS Driver Task Routine */ 
    DRV_USBHS_Tasks(DRV_USBHS_INDEX_0);
     
    /* USB Device layer tasks routine */ 
    USB_DEVICE_Tasks(USB_DEVICE_INDEX_0);
    
    if( usbDeviceThisInstance->appUsbTask != NULL )
    {
        usbDeviceThisInstance->appUsbTask();
    }
    
    if( usbDeviceThisInstance->appUsbExternalTask != NULL )
    {
        usbDeviceThisInstance->appUsbExternalTask();
    }    
}
#else
void UTaskHandler(void)
{
USB_DEVICE_OBJ * usbDeviceThisInstance;
