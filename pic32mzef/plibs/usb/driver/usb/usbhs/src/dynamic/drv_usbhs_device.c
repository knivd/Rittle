...
void DRV_USBHS_DEVICE_AddressSet(DRV_HANDLE handle, uint8_t address)
{
    ...
    {
        USB_SYS_DEBUG_MESSAGE("Driver Handle is invalid");
        return;
    }
...
{   
    if(DRV_HANDLE_INVALID == handle)
    {
        USB_SYS_DEBUG_MESSAGE("USB Driver: Invalid Handle");
        return 0;
    }
...
)
{
    ...
    {
        USB_SYS_DEBUG_MESSAGE("Unsupported endpoint");
    ...
    else
    {
            ...
            (hDriver->isInInterruptContextUSBDMA == false))
        {
            ...
            {
                USB_SYS_DEBUG_MESSAGE("Mutex lock failed");
        ...
        if(mutexLock == true)
        {
            ...
            {
                USB_SYS_DEBUG_MESSAGE("Mutex unlock failed");
            }
        }
    }
...
)
{
    ...
    if((DRV_HANDLE_INVALID == handle))
    {
        USB_SYS_DEBUG_MESSAGE("Invalid handle");
        return USB_ERROR_PARAMETER_INVALID;
    }
    ...
    if(irp->status > USB_DEVICE_IRP_STATUS_SETUP)
    {
        ...
        USB_SYS_DEBUG_MESSAGE("Device IRP is already in use");
    ...
    if(endpoint >= DRV_USBHS_ENDPOINTS_NUMBER)
    {
        USB_SYS_DEBUG_MESSAGE("Endpoint is not provisioned for");
        return USB_ERROR_DEVICE_ENDPOINT_INVALID;
    }
    ...
    if(remainder == 0)
    {
        ...
        if(USB_DATA_DIRECTION_DEVICE_TO_HOST == direction)
        {
            if(((irp->flags & USB_DEVICE_IRP_FLAG_DATA_COMPLETE) 
                ...
                (irp->size != 0)&&(irp->size != endpointObj->maxPacketSize))
            {
...
)
{
    ...
    {
        USB_SYS_DEBUG_MESSAGE("IRP is invalid");
        returnValue = USB_ERROR_PARAMETER_INVALID;
    }
    else
    {
...
void _DRV_USBHS_DEVICE_Tasks_ISR(DRV_USBHS_OBJ * hDriver)
{
    ...
    if(!hDriver->isOpened)
    {
        ...
        USB_SYS_DEBUG_MESSAGE("Driver does not have a client");
        return;
    }
    ...
    if(hDriver->pEventCallBack == NULL)
    {
        ...
        USB_SYS_DEBUG_MESSAGE("Driver needs a event handler");
        return;
    }
            ...
            if(irp != NULL)
            {
                ...
                if(irp->nPendingBytes == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            USB_SYS_DEBUG_MESSAGE("Was expecting status IRP");
                        }
        ...
        else if(ep0Status & USBHS_EP0_RXPKTRDY)
        {
                    ...
                    DRV_USBHS_DEVICE_EP0_STATE_EXPECTING_SETUP_FROM_HOST)
            {
                ...
                if(irp == NULL)
                {
                    ...
                    USB_SYS_DEBUG_MESSAGE("No IRP to receive setup packet!");
                    Nop();
                    Nop();
                }
                else
