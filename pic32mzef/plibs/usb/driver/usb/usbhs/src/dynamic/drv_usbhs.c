...
)
{
    ...
    {
        USB_SYS_DEBUG("Increase the value of DRV_USBHS_INSTANCES_NUMBER");
    ...
    if(gDrvUSBObj[drvIndex].inUse == true)
    {
        ...
        USB_SYS_DEBUG("Instance already in use");
        return SYS_MODULE_OBJ_INVALID;
    }
...
//    drvObj->rootHubInfo.portIndication = usbInit->portIndication;
    ...
    {
        /* For Host the ID pin needs to be pulle down (PORT_F, BIT_3) */
        *(&CNPDBSET + (0x05 - 1) * 0x40) = 1<<3;
    }
...
//            hDriver->timerHandle = DRV_USBHS_TMR_DelayMS(_DRV_USBHS_MODULE_RESET_DURATION);
 
//            if(hDriver->timerHandle != DRV_USB_TMR_HANDLE_INVALID)
            {
...
//            if(DRV_USBHS_TMR_DelayStatusGet(hDriver->timerHandle)) 
            {
                ...
                switch(hDriver->operationMode)
                {
                    ...
                    case DRV_USBHS_OPMODE_OTG:
                        break;
                    default:
                        USB_SYS_DEBUG("What mode are you trying?");
                        break;
                }
...
// *****************************************************************************
/* Function:
    void DRV_USBHS_Disable( const SYS_MODULE_OBJ object )

  Summary:
    Dynamic impementation of DRV_USBHS_Disable system interface function.

  Description:
    This is the dynamic impementation of DRV_USBHS_Disable system interface
    function.
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                _DRV_USBHS_InterruptSourceDisable(drvObj->interruptSourceUSBDma);
                _DRV_USBHS_InterruptSourceClear(drvObj->interruptSourceUSBDma);
...
void DRV_USBHS_Close( DRV_HANDLE client )
{
    ...
    {
        USB_SYS_DEBUG("Bad Client Handle");
        return;
    }
    ...
    if(!(hDriver->isOpened))
    {
        USB_SYS_DEBUG("Invalid client handle");
        return;
    }
...
void DRV_USBHS_Timer_Tick_ISR( SYS_MODULE_OBJ object )
{
    /* Start of local variables */
    DRV_USBHS_OBJ * 	hDriver;
    /* end of local variables */
...
)
{
    ...
    {
        USB_SYS_DEBUG("Bad Client or client closed");
    }
    else
    {
