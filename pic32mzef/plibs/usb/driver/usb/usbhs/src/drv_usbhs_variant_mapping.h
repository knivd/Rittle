...
#define USBHS_INT_SourceEnable( source ) \
        PLIB_USBHS_InterruptSourceEnable( USBHS_ID_0,source )

#define USBHS_INT_SourceDisable( source ) \
        PLIB_USBHS_INT_SourceDisable( USBHS_ID_0,source )

#define USBHS_INT_SourceStatusClear( source ) \
        PLIB_USBHS_InterruptSourceFlagClear( USBHS_ID_0,source )

#define USBHS_INT_SourceStatusGet(source) \
        PLIB_USBHS_InterruptSourceFlagGet( USBHS_ID_0, source )

#define USBHS_INT_SourceStatusSet( source ) \
        PLIB_USBHS_InterruptSourceFlagSet( USBHS_ID_0,source )
 ...
 * ASCII codes for D and A respectively. */
    ...
    #define _DRV_USBHS_NonPersistentInterruptSourceClear(source)    USBHS_INT_SourceStatusClear( source )
#else
    /* The interrupt flag will be cleared after the source events have been
     * processed. */
    #define _DRV_USBHS_PersistentInterruptSourceClear(source)       USBHS_INT_SourceStatusClear( source )
    #define _DRV_USBHS_NonPersistentInterruptSourceClear(source)
#endif

#if (DRV_USBHS_INTERRUPT_MODE == true)

    #define _DRV_USBHS_InterruptSourceEnable(source)        USBHS_INT_SourceEnable( source )
    #define _DRV_USBHS_InterruptSourceDisable(source)       USBHS_INT_SourceDisable( source )
    #define _DRV_USBHS_InterruptSourceClear(source)         USBHS_INT_SourceStatusClear( source )
    #define _DRV_USBHS_InterruptSourceStatusGet(source)     USBHS_INT_SourceStatusGet( source )
    #define _DRV_USBHS_InterruptSourceStatusSet(source)     USBHS_INT_SourceStatusSet( source )
