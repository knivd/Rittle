...
#include <xc.h>
#include "../peripheral_mz/common.h"
...
#define waitNVMCONnWR() { while(NVMCONbits.WR); }   // Wait for WR bit to clear

#define _NVM_UNLOCK_KEY1    0xAA996655
#define _NVM_UNLOCK_KEY2    0x556699AA

/* NVM Flash Memory programming keys */
typedef enum {

    NVM_UNLOCK_KEY1 = _NVM_UNLOCK_KEY1,
    NVM_UNLOCK_KEY2 = _NVM_UNLOCK_KEY2

} NVM_UNLOCK_KEYS;

typedef enum {

    NVM_ROW_SIZE = (4 * ROW_SIZE),
    NVM_PAGE_SIZE = (4 * PAGE_SIZE)
...
/*********************************************************************
  ...
  Note:            None
 ********************************************************************/
#define mNVMUnlockOpLockLLD(op)    do{int intStat=INTDisableInterrupts(); \
            NVMKEY = 0; NVMKEY = _NVM_UNLOCK_KEY1; NVMKEY = _NVM_UNLOCK_KEY2; \
            (op); \
            NVMKEY = 0x33333333; \
...
int dma_susp; // storage for current DMA state
    ...
    NVMKEY = 0x0;
    NVMKEY = _NVM_UNLOCK_KEY1;
    NVMKEY = _NVM_UNLOCK_KEY2;
